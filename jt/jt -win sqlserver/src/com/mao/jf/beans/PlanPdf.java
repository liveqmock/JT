...
public class PlanPdf {
	...
	public  PdfPTable createHeaderTable() throws DocumentException, IOException {
		...
		paragraph.add("        ");
		chunk=new Chunk(String.format("排产日期：%1ty年%1tm月%1td日", plan.getStartDate()),underLine);
		chunk.setLineHeight(30);
		paragraph.add(chunk);

		paragraph.add("        ");
		chunk=new Chunk(String.format("计时完成时间：%1ty年%1tm月%1td日",plan.getEndDate()),underLine);
		chunk.setLineHeight(30);
		paragraph.add(chunk);
	...
	private  PdfPTable createBodyTable() throws DocumentException, IOException {
		...
		table.setWidthPercentage(100);
		table.setWidths(new int[]{3,3,5,3,2,3,2,3,2,2,3,2,3,3});
