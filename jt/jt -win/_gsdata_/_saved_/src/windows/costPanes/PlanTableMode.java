...
public class PlanTableMode extends AbstractTableModel {
		...
		default:
			if(columnIndex<columnNames.size()-2){
				...
				if(columnIndex==columnNames.size()-2)
					...
					return plan.getBill().getPlanCostUnit();
			}
		}
		return "unknow";
	}
	...
	public Class<?> getColumnClass(int columnIndex) {
		...
		default:
			if(columnIndex<columnNames.size()-2){
				return columnIndex%2==1?int.class:double.class;
			}else{
				return double.class;
			}
		}
		...
		if(columnIndex==7) plan.setProduceDate((Date) aValue);
			...
			if(columnIndex>8 ||columnIndex<columnNames.size()-2) {
				if(columnIndex%2==1&&aValue!=null){					
					OperationPlan operationPlan=	plan.getOperationPlans().get(operationNames.get((columnIndex-8)/2));
					if(operationPlan.getOut()&& ((int) aValue)>0)
						operationPlan.setUseTime(1);
					else
						operationPlan.setUseTime( (int) aValue);
