...
final class HostNameValidator implements Validator<String> {
	...
	public boolean validate(Problems problems, String compName, String model) {
		...
		if (model.charAt(0) == '.' || model.endsWith(".")) { // NOI18N
		...
		if (model.endsWith("-") || model.charAt(0) == '-') {
			problems.add(NbBundle.getMessage(HostNameValidator.class,
					"INVALID_HOST_NAME", compName, model)); // NOI18N
			return false;
		}
				...
				if (labelAndPort.length == 1) {
					...
					if (label.endsWith(":")) {
						problems.add(NbBundle.getMessage(
								HostNameValidator.class, "TOO_MANY_COLONS",
								compName, label)); // NOI18N
						return false;
					}
					try {
						...
						problems.add(NbBundle.getMessage(
								HostNameValidator.class, "INVALID_PORT",
								compName, labelAndPort[1])); // NOI18N
						return false;
					}
	...
	private boolean checkHostPart(String label, Problems problems,
			String compName) {
		boolean result = true;
		if (label.length() > 63) {
			problems.add(NbBundle.getMessage(HostNameValidator.class,
		...
		if (result) {
			...
			if (result) {
				if (result) {
					result = new EncodableInCharsetValidator().validate(
							problems, compName, label);
					if (result) {
						for (char c : label.toLowerCase().toCharArray()) {
							...
							problems.add(NbBundle.getMessage(
									...
									"BAD_CHAR_IN_HOSTNAME", new String(
											new char[] { c }))); // NOI18N
							result = false;
						}
					}
