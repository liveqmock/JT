...
public class BeanManager {
	public static BeanManager BM=new BeanManager();
	private EntityManager em;
	private EntityManagerFactory emf;
	
	public BeanManager() {
		emf = Persistence.createEntityManagerFactory( "DataCenter" );
	...
	public <T> List<T> getBeans(Class<T> beanClass,String whereString) {
		...
		return em.createQuery( "FROM "+beanClass.getSimpleName() +" as a "+whereString,beanClass ).getResultList();

	}
	public <T> List<T> getBeans(Class<T> beanClass,String whereString,Object... objects) {

		TypedQuery<T> query = em.createQuery( "FROM "+beanClass.getSimpleName() +" as a "+whereString,beanClass );
		if(objects!=null){
			for(int i=0;i<objects.length;i++)
				query.setParameter(i+1, objects[i]);
		}
	...
	public <T> T  getBean(Class<T> beanClass,String whereString) throws NoResultException  {
		return em.createQuery( "FROM "+beanClass.getSimpleName() +" as a "+whereString,beanClass ).getSingleResult();
