...
public class BeanTablePane<T> extends JPanel {
	...
	public BeanTablePane(Vector<T> beans) {
		this(beans, null);
	}

	public BeanTablePane(Vector<T> beans, String filterColoums[]) {
		super();
		this.beans = beans;
		this.filterColoums = filterColoums;
		table = new JXTable(new BeanTableModel<>(beans));
	...
	public void setFilter() {
		if (filterColoums != null && filterColoums.length > 0
			...
			LinkedList<String> heads = ((BeanTableModel<T>) table.getModel())
					.getHeads();
				...
				if (filterColoums != null || filterColoums.length > 0) {
					...
					for (int i = 0; i < filterColoums.length; i++) {
						...
						for (int c = 0; c < heads.size(); c++) {
							if (heads.get(c).equals(filterColoums[i])) {
								TreeSet<String> colValues = new TreeSet<>();
								colValues.add("");
								for (T t : beans) {
									try {
										@SuppressWarnings("unchecked")
										Object object = ((BeanTableModel<T>) table
												.getModel()).getMethods()
												.get(c).invoke(t);
										if (object != null)
	...
	public void addNew(T t) {
		((BeanTableModel<T>) getTable().getModel()).insertRow(0, t);
	}
