...
public class BillPanel extends JSplitPane {
	...
	private void outAdminEnable() {
		billidFld.setEnabled(User.loginUser.getLevel() == 0);
		billNoFld.setEnabled(User.loginUser.getLevel() == 0);
		itemNoFld.setEnabled(User.loginUser.getLevel() == 0);
		priceFld.setEnabled(User.loginUser.getLevel() == 0);
		picNoFld.setEnabled(User.loginUser.getLevel() == 0);
		numFld.setEnabled(User.loginUser.getLevel() == 0);
		customCombox.setEnabled(User.loginUser.getLevel() == 0);
		contactManCombox.setEnabled(User.loginUser.getLevel() == 0);
		billcreateDate.setEnabled(User.loginUser.getLevel() == 0);
		requestDate.setEnabled(User.loginUser.getLevel() == 0);
		billedDate.setEnabled(User.loginUser.getLevel() == 0);
		outPriceFld.setEnabled(User.loginUser.getLevel() == 0);
		outBillNoFld.setEnabled(User.loginUser.getLevel() == 0);
		addCustomButton.setEnabled(User.loginUser.getLevel() == 0);
		addOutCustomBt.setEnabled(User.loginUser.getLevel() == 0);

	}

	private void outEnable(boolean b) {
		outBilledDate.setEnabled(b && User.loginUser.getLevel() == 0
				&& outBillNoFld.getText() != null
				&& !outBillNoFld.getText().equals(""));
		outBillNoFld.setEnabled(b && User.loginUser.getLevel() == 0);
		outgetDate.setEnabled(b);
		outNumFld.setEnabled(b);
		outPriceFld.setEnabled(b && User.loginUser.getLevel() == 0);
	...
	private void addValitors() {
		outBilledDate.setEnabled(User.loginUser.getLevel() == 0
				&& outBillNoFld.getText() != null
				&& !outBillNoFld.getText().equals(""));
		billedDate.setEnabled(User.loginUser.getLevel() == 0
	...
	private void createContents() {
		...
		panel.add(priceFld, "4, 10, fill, default");

		JLabel label_2 = new JLabel(
				"\u8981\u6C42\u4EA4\u8D27\u65E5\u671F\uFF1A");
		panel.add(label_2, "6, 10");

		requestDate = new JXDatePicker();
		requestDate.setFormats(new String[] { "yyyy-MM-dd" });
		panel.add(requestDate, "8, 10");
		...
		panel.add(billgetDate, "4, 12");

		JLabel label_9 = new JLabel("\u5165\u5E93\uFF1A");
		panel.add(label_9, "6, 12, right, default");

		warehousCheckBox = new JCheckBox("\u5DF2\u5165\u5E93");
		panel.add(warehousCheckBox, "8, 12");

		JLabel label_101 = new JLabel("开票日期：");
		panel.add(label_101, "6, 14");

		billedDate = new JXDatePicker();
		billedDate.setFormats(new String[] { "yyyy-MM-dd" });
		panel.add(billedDate, "8, 14");

		JLabel label_91 = new JLabel("发票号码：");
		panel.add(label_91, "2, 14, right, default");

		billNoFld = new JTextField();
		panel.add(billNoFld, "4, 14");

		JLabel label_3 = new JLabel("\u5916\u534F\u5BA2\u6237\uFF1A");
		panel.add(label_3, "2, 16");

		JPanel panel_3 = new JPanel();
		panel.add(panel_3, "4, 16, 5, 1, fill, fill");
		...
		panel.add(label_6, "2, 18, right, default");

		outPriceFld = new JTextField();
		outPriceFld.setColumns(10);
		panel.add(outPriceFld, "4, 18, fill, default");
